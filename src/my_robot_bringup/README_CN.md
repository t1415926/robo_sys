...
`pointlio_nav_bag.launch.py` 默认使用 `my_robot_navigation/config/nav2_pointcloud_params.yaml`，local costmap 会订阅 `/cloud_registered_body` 作为实时点云障碍层；`/Laser_map` 仍通过投影节点生成全局 `/map`。启动文件会发布 `base_footprint -> body` 的初始静态 TF，实车上应按标定结果替换。
