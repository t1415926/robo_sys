...
KD_TREE<PointType> prior_ikdtree;
...
static void nearestSearchInActiveMaps(PointType point, int k_nearest, PointVector &nearest_points,
									  vector<float> &point_distance, float max_dist)
{
	PointVector dynamic_points;
	PointVector prior_points;
	vector<float> dynamic_distance;
	vector<float> prior_distance;

	if (ikdtree.Root_Node != nullptr)
	{
		ikdtree.Nearest_Search(point, k_nearest, dynamic_points, dynamic_distance, max_dist);
	}
	if (prior_map_loaded && prior_ikdtree.Root_Node != nullptr)
	{
		prior_ikdtree.Nearest_Search(point, k_nearest, prior_points, prior_distance, max_dist);
	}
	...
	size_t prior_idx = 0;
	while (nearest_points.size() < static_cast<size_t>(k_nearest) &&
		   (dynamic_idx < dynamic_points.size() || prior_idx < prior_points.size()))
	{
		bool take_dynamic = prior_idx >= prior_points.size() ||
							(dynamic_idx < dynamic_points.size() &&
							 dynamic_distance[dynamic_idx] <= prior_distance[prior_idx]);
		if (take_dynamic)
		{
			nearest_points.emplace_back(dynamic_points[dynamic_idx]);
			point_distance.emplace_back(dynamic_distance[dynamic_idx]);
			dynamic_idx++;
		}
		else
		{
			nearest_points.emplace_back(prior_points[prior_idx]);
			point_distance.emplace_back(prior_distance[prior_idx]);
			prior_idx++;
		}
	}
}
...
void h_model_input(state_input &s, esekfom::dyn_share_modified<double> &ekfom_data)
{
	...
	for (int j = 0; j < time_seq[k]; j++)
	{
		...
		
		{
			...
			nearestSearchInActiveMaps(point_world_j, NUM_MATCH_POINTS, points_near, pointSearchSqDis, 2.236); //1.0); //, 3.0); // 2.236;
...
void h_model_output(state_output &s, esekfom::dyn_share_modified<double> &ekfom_data)
{
	...
	for (int j = 0; j < time_seq[k]; j++)
	{
		...
		p_world << point_world_j.x, point_world_j.y, point_world_j.z;
		{
			...
			nearestSearchInActiveMaps(point_world_j, NUM_MATCH_POINTS, points_near, pointSearchSqDis, 2.236); 
